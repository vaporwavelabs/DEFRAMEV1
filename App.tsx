...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const restoreSession = async () => {
      ...
      if (sessionData?.vfs_json) {
        setVfs(sessionData.vfs_json);
        addLog("KERNEL: Virtual File System restored.", "success");
      }

      if (logData?.length) {
        setLogs(logData);
        setErrors(logData.filter(l => l.level === 'error'));
        addLog("KERNEL: Historical logs synchronized.", "success");
      }
      
      if (chatData?.length) {
        setChatHistory(chatData);
        addLog("KERNEL: Neural Bridge history restored.", "success");
      }
      
      addLog("KERNEL: System is online and fully operational.", "success");
      setIsRestoring(false);
    };
    restoreSession();
  }, [addLog]);
  ...
  useEffect(() => {
    const monitorLoop = setInterval(() => {
      ...
      setCurrentMetrics({ cpuUsage: estimatedCpu, memoryUsed, heapTotal, timestamp: Date.now() });
    }, 1000);
      ...
      if (parsed.files) {
        ...
        setVfs(prev => {
          ...
          parsed.files.forEach((f: any) => {
            const existing = newVfs[0].children?.find(node => node.path === f.path);
            if (existing) existing.content = f.content;
            else newVfs[0].children?.push({ name: f.path.split('/').pop() || f.path, path: f.path, type: 'file', content: f.content });
          });
          return newVfs;
        });
        persistAppState(newVfs);
  ...
  if (isRestoring) {
    return (
        <div className="h-screen w-screen bg-black flex flex-col items-center justify-center text-center gap-8 font-mono text-green-500">
            <HardDriveDownload size={80} className="animate-pulse" />
            <h1 className="text-3xl font-black tracking-[0.5em] uppercase text-white">RESTORING KERNEL</h1>
            <p className="text-sm opacity-50 max-w-sm">Synchronizing with cloud storage... Establishing secure connection to Supabase persistence layer.</p>
        </div>
    );
  }
